function results=apply_baselines_caltech256(sig,yt,opts)
% Runs L1-homotopy, SpaRSA, and Kalman filtering
%
% Inputs
% -signal sig (size 128*256 x 1) 
% -noisy compressed observations ycat (size 128 x 256/opts.R)
% -opts is a struct of options, with fields
%   R - compression factor for observations
%   N - dimension of time-varying observations (rows of image)
%   P - size of working window
%   PHI - pre-defined sensing matrix, constant for all time
%
% Outputs
% results is a struct of results, with fields
%
% Written by Scott Wisdom (swisdom@uw.edu) June 2016
%
% Modified version of
% demo_KalmanRWT
%
...
sType = 'image'; % {'heavisine', 'pcwreg', 'blocks','pcwPoly'}
...
% streaming window
P = opts.P; % size of the working window is P*N
...
% % check that y equals PHI*s_{3:3+P-1}:
% st=reshape(sig(:),[N,sig_length/N]);
% PHIst=opts.PHI*st;
% PHIst_seg=PHIst(:,3:3+P-1);
% sum((PHIst_seg(:)-y).^2)
e = reshape(yt(3:3+P-1,:).',[M*P,1])-y;
y = y+e;
...
while ~done
    ...
    % st=reshape(sig(:),[N,sig_length/N]);
    % PHIst=opts.PHI*st;
    % PHIst_seg=PHIst(:,pidx);
    % sum((PHIst_seg(:)-y(end-M+1:end)).^2)
    e(end-M+1:end) = yt(pidx,:).'-y(end-M+1:end);
    pidx=pidx+1;
...
fprintf('\n');
SIM_stack=SIM_stack(find(~cellfun(@isempty,SIM_stack)));
...
%% store results
results.mse=[err_l1homotopy*norm(sig_vec(st_ind+1:N*sim))^2,err_sparsa*norm(sig_vec(st_ind+1:N*sim))^2,err_kalman*norm(sig_vec(st_ind+1:N*sim))^2];
results.ser=-10*log10([err_l1homotopy,err_sparsa,err_kalman]);
results.f=0;
results.iters=[sum(iter_streamingRWT,2),sum(iter_sparsa,2)];
results.sigh_vec=sigh_vec;
