...
def rms_prop(learning_rate, parameters, gradients, idx_project=None):        
    rmsprop = [theano.shared(np.float32(1e-3)*np.ones_like(p.get_value()).astype(np.float32)) for p in parameters]
    ...
    new_rmsprop = [np.float32(0.9) * T.cast(vel,'float32') + np.float32(0.1) * (T.cast(g,'float32')**2) for vel, g in zip(rmsprop, gradients)]

    updates1 = zip(rmsprop, new_rmsprop)
    updates2 = [(p, p - np.float32(learning_rate) * T.cast(g,'float32') / T.sqrt(T.cast(rms,'float32'))) for 
